export const navbarData = [
  {
    routerLink: "general",
    icon: "../assets/order-management/Order-Management.png",
    label: "General",
    subMenus: [
      { routerLink: "currency", icon: null, label: "Currency", pinned: true },
      { routerLink: "country", icon: null, label: "Country", pinned: true },
      {
        routerLink: "garment-type",
        icon: null,
        label: "Garment Type",
        pinned: true,
      },
      // {
      //   routerLink: "currency-exchange",
      //   icon: null,
      //   label: "Currency Exchange",
      //   pinned: false,
      // },
      {
        routerLink: "currency-conversion",
        icon: "",
        label: "Currency Conversion",
        pinned: false,
      },
      {
        routerLink: "unit",
        icon: null,
        label: "Unit",
        pinned: true,
      },
      {
        routerLink: "bank",
        icon: null,
        label: "Bank",
        pinned: true,
      },
      {
        routerLink: "basis",
        icon: null,
        label: "Basis",
        pinned: true,
      },
      {
        routerLink: "season",
        icon: null,
        label: "Season",
        pinned: true,
      },
    ],
    tag: "general",
  },
  {
    routerLink: "ordermanagementref",
    icon: "../assets/order-management/Order-Management.png",
    label: "Order Management Reference",
    subMenus: [
      {
        routerLink: "buyers",
        icon: null,
        label: "Buyer",
        pinned: true,
      },
      { routerLink: "supplier", icon: null, label: "Supplier", pinned: false },
      {
        routerLink: "port-destination",
        icon: null,
        label: "Country wise Dest./Port",
        pinned: false,
      },
      {
        routerLink: "item-feature",
        icon: null,
        label: "Item Feature",
        pinned: false,
      },
      {
        routerLink: "order-item-feature",
        icon: null,
        label: "Order Item Feature",
        pinned: false,
      },
      {
        routerLink: "garment-type-items",
        icon: null,
        label: "Garment Type Item Requirements",
        pinned: false,
      },
      {
        routerLink: "additional-cost",
        icon: null,
        label: "Additional Cost",
        pinned: false,
      },
      {
        routerLink: "sub-contractor",
        icon: null,
        label: "Sub Contractor",
        pinned: false,
      },
      {
        routerLink: "stock-reference",
        icon: null,
        label: "Stock Reference",
        pinned: false,
      },
      {
        routerLink: "order-item-catalog",
        icon: null,
        label: "Order Items Catalog",
        pinned: false,
      },
    ],
    tag: "ordermanagementref",
  },

  {
    routerLink: "ordermanagement",
    icon: "../assets/order-management/Order-Management.png",
    label: "Order Management",
    subMenus: [
      {
        routerLink: "po",
        icon: "../assets/order-management/Order-Management.png",
        label: "Order Confirmation Routine",
        pinned: true,
      },
      {
        routerLink: "material-consumption",
        icon: "../assets/order-management/Order-Management.png",
        label: "Material Consumptions",

        pinned: true,
      },
      {
        routerLink: "trim-sheet-approval",
        icon: "../assets/order-management/Order-Management.png",
        label: "Approve Trim Sheet",
        pinned: false,
      },
      {
        routerLink: "additional",
        icon: "../assets/order-management/Order-Management.png",
        label: "Additional Costs per Garment",
        pinned: false,
      },
      {
        routerLink: "subcont",
        icon: "../assets/order-management/Order-Management.png",
        label: "Sub Contracts",
        pinned: false,
      },
      {
        routerLink: "supplier-po",
        icon: "../assets/order-management/Order-Management.png",
        label: "Purchase Order Entry",
        pinned: true,
      },
      {
        routerLink: "stylewise-events",
        icon: "../assets/order-management/Order-Management.png",
        label: "Style-wise Events",
        pinned: false,
      },
    ],
    tag: "ordermanagement",
  },
  {
    routerLink: "generalinventory",
    icon: "../assets/inventory/Inventory-Management.png",
    label: "General Inventory",
    subMenus: [
      {
        routerLink: "general-srn",
        icon: null,
        label: "Stores Requisition Note",
        pinned: true,
      },
      {
        routerLink: "general-gin",
        icon: null,
        label: "Goods Issue        Note",
        pinned: false,
      },
      {
        routerLink: "general-grn",
        icon: null,
        label: "Goods Receive     Note",
        pinned: false,
      },
      {
        routerLink: "general-rtn",
        icon: null,
        label: "Goods Return Note",
        pinned: false,
      },
      {
        routerLink: "general-gtn",
        icon: null,
        label: "Goods Transfer     Note",
        pinned: false,
      },
      {
        routerLink: "general-ogtn",
        icon: null,
        label: "Goods Transfer     Note (Orders)",
        pinned: false,
      },
      {
        routerLink: "general-srtn",
        icon: null,
        label: "Supplier Return    Note",
        pinned: false,
      },
      {
        routerLink: "general-dgn",
        icon: null,
        label: "Damaged Goods      Note",
        pinned: false,
      },
      {
        routerLink: "ain",
        icon: null,
        label: "Additional issue   Note",
        pinned: false,
      },
      // {
      //   routerLink: "dtn",
      //   icon: null,
      //   label: "Direct Transfer    Note",
      //   pinned: false,
      // },
      {
        routerLink: "general-san",
        icon: null,
        label: "Stock Adjustments  Note ",
        pinned: false,
      },
      {
        routerLink: "general-po",
        icon: null,
        label: "Purchase Order     Entry",
        pinned: false,
      },
      {
        routerLink: "general-stock-master",
        icon: null,
        label: "Stock Master       Entry",
        pinned: false,
      },
    ],
  },
  {
    routerLink: "orderwiseinventory",
    icon: "../assets/order-management/Order-Management.png",
    label: "Orderwise Inventory",
    subMenus: [
      {
        routerLink: "srn",
        icon: null,
        label: "Stores Requisition Note",
        pinned: false,
      },
      // {
      //   routerLink: "gin",
      //   icon: null,
      //   label: "Goods Issue Note",
      //   pinned: false,
      // },
      {
        routerLink: "gin-cascade",
        icon: null,
        label: "Goods Issue Note",
        pinned: false,
      },
      // {
      //   routerLink: "grn",
      //   icon: null,
      //   label: "Goods Received Note",
      //   pinned: false,
      // },
      {
        routerLink: "grn-cascade",
        icon: null,
        label: "Goods Received Note",
        pinned: false,
      },
      // {
      //   routerLink: "grcn",
      //   icon: null,
      //   label: "Goods Receive Note",
      //   pinned: false,
      // },
      {
        routerLink: "rtn",
        icon: null,
        label: "Goods Return Note",
        pinned: false,
      },
      {
        routerLink: "gtn",
        icon: null,
        label: "Goods Transfer Note",
        pinned: false,
      },
      {
        routerLink: "dtn",
        icon: null,
        label: "Direct Goods Transfer Note",
        pinned: false,
      },
      {
        routerLink: "supplier-return-note",
        icon: null,
        label: "Supplier Return Note",
        pinned: false,
      },
      {
        routerLink: "dgn",
        icon: null,
        label: "Damaged Goods Note",
        pinned: false,
      },
      {
        routerLink: "ain",
        icon: null,
        label: "Additional issue Note",
        pinned: false,
      },
      {
        routerLink: "arn",
        icon: null,
        label: "Additional Goods Receipts Note",
        pinned: false,
      },
      {
        routerLink: "dtn",
        icon: null,
        label: "Direct Transfer Note",
        pinned: false,
      },
      {
        routerLink: "san",
        icon: null,
        label: "Stock Adjustments Note ",
        pinned: false,
      },
    ],
    tag: "orderwiseinventory",
  },
  {
    routerLink: "productioncontrol",
    icon: "../assets/production/Inventory-Management.png",
    label: "Production Control",
    subMenus: [
      {
        routerLink: "daily-production-time-ticket",
        icon: null,
        label: "Daily Production Time Ticket",
        pinned: false,
      },
      {
        routerLink: "daily-production-entry",
        icon: null,
        label: "Actual Production entry ",
        pinned: false,
      },
      {
        routerLink: "estimated-production-entry",
        icon: null,
        label: "Estimated Production entry",
        pinned: false,
      },
      {
        routerLink: "production-line-allocation",
        icon: null,
        label: "Production Line Allocation",
        pinned: false,
      },
      {
        routerLink: "style-component-breakdown",
        icon: null,
        label: "Style wise Component Breakdown",
        pinned: false,
      },
      {
        routerLink: "style-operation-breakdown",
        icon: null,
        label: "Style wise Operation Breakdown",
        pinned: false,
      },
      {
        routerLink: "end-of-production-confirmation",
        icon: null,
        label: "End of Production Confirmation",
        pinned: false,
      },
      {
        routerLink: "estimated-production-line-allocation",
        icon: null,
        label: "Est. Prod. Line Allocation",
        pinned: false,
      },
      {
        routerLink: "production-progress-graph",
        icon: null,
        label: "Production Progress Graph",
        pinned: false,
      },
    ],
    tag: "productioncontrol",
  },
  {
    routerLink: "productionreports",
    icon: "../assets/production/Inventory-Management.png",
    label: "Reports - Production",
    subMenus: [
      {
        routerLink: "production-schedule-report",
        icon: null,
        label: "Production Schedule",
        pinned: false,
      },
      {
        routerLink: "production-summary-daily-report",
        icon: null,
        label: "Production Summary (Daily)",
        pinned: false,
      },
      {
        routerLink: "production-summary-monthly-report",
        icon: null,
        label: "Production Summary (Monthly)",
        pinned: false,
      },
      {
        routerLink: "production-summary-monthly-overview-report",
        icon: null,
        label: "Production Summary (Monthly) - Simplified",
        pinned: false,
      },
      {
        routerLink: "production-summary-style-wise-report",
        icon: null,
        label: "Production Summary (Style Wise)",
        pinned: false,
      },
      {
        routerLink: "production-summary-style-wise-detailed-report",
        icon: null,
        label: "Production Summary (Style Wise) - Detailed",
        pinned: false,
      },
      {
        routerLink: "line-production-summary-report",
        icon: null,
        label: "Line Production Summary",
        pinned: false,
      },
      {
        routerLink: "operation-breakdown-report",
        icon: null,
        label: "Operation Breakdown",
        pinned: false,
      },
      {
        routerLink: "manpower-requirement-report",
        icon: null,
        label: "Manpower Requirement",
        pinned: false,
      },
      {
        routerLink: "daily-employee-efficiency-report",
        icon: null,
        label: "Employee Efficiency (Daily)",
        pinned: false,
      },
      {
        routerLink: "monthly-employee-efficiency-report",
        icon: null,
        label: "Employee Efficiency (Monthly)",
        pinned: false,
      },
      {
        routerLink: "line-efficiency-report",
        icon: null,
        label: "Production Line Efficiency",
        pinned: false,
      },
      {
        routerLink: "estimated-production-schedule-report",
        icon: null,
        label: "Estimated Production Schedule",
        pinned: false,
      },
      {
        routerLink: "production-analysis-summary-report",
        icon: null,
        label: "Production Analysis Summary (Style)",
        pinned: false,
      },
    ],
    tag: "productionreports",
  },
  {
    routerLink: "productionreference",
    icon: "",
    label: "Production Reference",
    subMenus: [
      {
        routerLink: "production-line",
        icon: null,
        label: "Line",
        pinned: false,
      },
      {
        routerLink: "operation",
        icon: null,
        label: "Operation Reference",
        pinned: false,
      },
      {
        routerLink: "machine-type",
        icon: null,
        label: "Machine Type Reference",
        pinned: false,
      },
      {
        routerLink: "garment-component",
        icon: null,
        label: "Component Reference",
        pinned: false,
      },
      {
        routerLink: "non-productive-hour-code",
        icon: null,
        label: "Non-productive Nature",
        pinned: false,
      },
      {
        routerLink: "employee",
        icon: null,
        label: "Employee Reference",
        pinned: false,
      },
      {
        routerLink: "holiday",
        icon: null,
        label: "Production Calendar",
        pinned: false,
      },
    ],
    tag: "productionreference",
  },
  {
    routerLink: "",
    icon: "",
    label: "",
    subMenus: [],
  },
  {
    routerLink: "general-inventory-notes-reports",
    icon: "../assets/production/Inventory-Management.png",
    label: "Reports - General Inventory Notes",
    subMenus: [
      {
        routerLink: "general-strn-print",
        icon: null,
        label: "Stores Requisition Note - Print",
        pinned: false,
      },
      {
        routerLink: "general-gin-print",
        icon: null,
        label: "Goods Issue Note - Print",
        pinned: false,
      },
      {
        routerLink: "general-grn-print",
        icon: null,
        label: "Goods Received Note - Print",
        pinned: false,
      },
      {
        routerLink: "general-gtn-print",
        icon: null,
        label: "Goods Transfer Note - Print",
        pinned: false,
      },
      {
        routerLink: "general-ogtn-print",
        icon: null,
        label: "Goods Transfer Note (Orders) - Print",
        pinned: false,
      },
      {
        routerLink: "general-rtn-print",
        icon: null,
        label: "Goods Return Note - Print",
        pinned: false,
      },
      {
        routerLink: "general-dgn-print",
        icon: null,
        label: "Damaged Goods Note - Print",
        pinned: false,
      },
      {
        routerLink: "general-srtn-print",
        icon: null,
        label: "Supplier Return Note - Print",
        pinned: false,
      },
      {
        routerLink: "general-po-print",
        icon: null,
        label: "Purchase Order - Print",
        pinned: false,
      },
      {
        routerLink: "general-san-print",
        icon: null,
        label: "Stock Adjustment Note - Print",
        pinned: false,
      },
    ],
  },
  {
    routerLink: "general-inventory-reports",
    icon: "../assets/production/Inventory-Management.png",
    label: "Reports - General Inventory",
    subMenus: [
      {
        routerLink: "general-stock-status-report",
        icon: null,
        label: "Stock Status Report",
        pinned: true,
      },
      {
        routerLink: "general-stock-movement-report",
        icon: null,
        label: "Stock Movement Report",
        pinned: true,
      },
      {
        routerLink: "general-stock-valuation-report",
        icon: null,
        label: "Stock Valuation Report",
        pinned: true,
      },
      {
        routerLink: "general-stock-reorder-report",
        icon: null,
        label: "Stock Re-order Report",
        pinned: true,
      },
      {
        routerLink: "general-transaction-list-report",
        icon: null,
        label: "List of Transactions",
        pinned: true,
      },
      {
        routerLink: "general-purchase-order-list-report",
        icon: null,
        label: "List of P/O's",
        pinned: true,
      },
      {
        routerLink: "general-stock-summary-report",
        icon: null,
        label: "Stock Summary Report",
        pinned: true,
      },
      {
        routerLink: "general-grn-listing-report",
        icon: null,
        label: "GRN Listing",
        pinned: true,
      },
    ],
  },
  {
    routerLink: "orderwise-inventory-notes-reports",
    icon: "../assets/production/Inventory-Management.png",
    label: "Reports - Orderwise Inventory Notes",
    subMenus: [
      {
        routerLink: "strn-print",
        icon: null,
        label: "Stores Requisition Note - Print",
        pinned: false,
      },
      {
        routerLink: "gin-print",
        icon: null,
        label: "Goods Issue Note - Print",
        pinned: false,
      },
      {
        routerLink: "grn-print",
        icon: null,
        label: "Goods Received Note - Print",
        pinned: false,
      },
      {
        routerLink: "rtn-print",
        icon: null,
        label: "Goods Return Note - Print",
        pinned: false,
      },
      {
        routerLink: "gtn-print",
        icon: null,
        label: "Goods Transfer Note - Print",
        pinned: false,
      },
      {
        routerLink: "srn-print",
        icon: null,
        label: "Supplier Return Note - Print",
        pinned: false,
      },
      {
        routerLink: "dgn-print",
        icon: null,
        label: "Damaged Goods Note - Print",
        pinned: false,
      },
      {
        routerLink: "ain-print",
        icon: null,
        label: "Additional issue Note - Print",
        pinned: false,
      },
      {
        routerLink: "arn-print",
        icon: null,
        label: "Additional Goods Receipts Note - Print",
        pinned: false,
      },
      {
        routerLink: "dtn-print",
        icon: null,
        label: "Direct Transfer Note - Print",
        pinned: false,
      },
      {
        routerLink: "san-print",
        icon: null,
        label: "Stock Adjustments Note - Print",
        pinned: false,
      },
    ],
    tag: "orderwise-inventory-notes-reports",
  },
  {
    routerLink: "orderwise-inventory-reports",
    icon: "../assets/production/Inventory-Management.png",
    label: "Reports - Orderwise Inventory",
    subMenus: [
      {
        routerLink: "stock-movement-report",
        icon: null,
        label: "Stock Movement Report (for an Order)",
        pinned: true,
      },

      {
        routerLink: "stock-valuation-report",
        icon: null,
        label: "Stock Valuation Report",
        pinned: true,
      },
      {
        routerLink: "stock-valuation-monthly-report",
        icon: null,
        label: "Stock Valuation Report(monthly)",
        pinned: true,
      },
      {
        routerLink: "orderwise-stock-status-report",
        icon: null,
        label: "Stock Status Report",
        pinned: true,
      },
      {
        routerLink: "stock-movement-item",
        icon: null,
        label: "Stock Movement (for an Item) ",
        pinned: false,
      },
      {
        routerLink: "orderwise-transaction-list-report",
        icon: null,
        label: "List of Transactions",
        pinned: true,
      },
      {
        routerLink: "item-wise-stock-balance-report",
        icon: null,
        label: "Item-wise Stock Balances",
        pinned: true,
      },
      {
        routerLink: "raw-material-control-sheet-report",
        icon: null,
        label: "Raw Material Control Sheet",
        pinned: true,
      },
      {
        routerLink: "orderwise-stock-summary-report",
        icon: null,
        label: "Stock Summary Report - Basis wise",
        pinned: true,
      },
      {
        routerLink: "orderwise-grn-listing-report",
        icon: null,
        label: "GRN Listings ",
        pinned: true,
      },
    ],
    tag: "orderwise-inventory-reports",
  },
  // {
  //   routerLink: "quotaregister",
  //   icon: "../assets/order-management/Order-Management.png",
  //   label: "Quota register",
  //   subMenus: [
  //     { routerLink: "quotregister", icon: null, label: "Quota Register" },
  //     {
  //       routerLink: "quota-transfe-in",
  //       icon: null,
  //       label: "Quota Transfers In",
  //     },
  //     {
  //       routerLink: "quota-transfe-out",
  //       icon: null,
  //       label: "Quota Transfers Out",
  //     },
  //     {
  //       routerLink: "surrender-quota",
  //       icon: null,
  //       label: "Surrender of Quota",
  //     },
  //     { routerLink: "quota-cat-change", icon: null, label: "Category Change" },
  //     {
  //       routerLink: "quota-deffered",
  //       icon: null,
  //       label: "Transfer of Defered Quota",
  //     },
  //     { routerLink: "quota-reports", icon: null, label: "Reports" },
  //   ],
  //   tag: "quotaregister",
  // },
  {
    routerLink: "importexportdoc",
    icon: "../assets/order-management/Order-Management.png",
    label: "Import/Export Documentation",
    subMenus: [
      {
        routerLink: "ie-company-setup",
        icon: null,
        label: "Company Address Setup",
        pinned: false,
      },
      {
        routerLink: "commercial-invoice",
        icon: null,
        label: "Commercial (Export) Invoice",
        pinned: false,
      },
      {
        routerLink: "letter-of-credit",
        icon: null,
        label: "L/C Form",
        pinned: false,
      },
      {
        routerLink: "customs-declaration",
        icon: null,
        label: "CUSDEC I/II",
        pinned: false,
      },
    ],
    tag: "importexportdoc",
  },
  {
    routerLink: "iereference",
    icon: "../assets/order-management/Order-Management.png",
    label: "Import/Export Reference",
    subMenus: [
      {
        routerLink: "clearance-office",
        icon: null,
        label: "Clearance / Frontier Office",
        pinned: false,
      },
      {
        routerLink: "payment-term",
        icon: null,
        label: "Payment Terms",
        pinned: false,
      },
      {
        routerLink: "transport-mode",
        icon: null,
        label: "Transport Mode (TRPT)",
        pinned: false,
      },
      {
        routerLink: "document-type",
        icon: null,
        label: "Document Type",
        pinned: false,
      },
      {
        routerLink: "duty-tax-code",
        icon: null,
        label: "Duty / Tax Code",
        pinned: false,
      },
      {
        routerLink: "tax-base-code",
        icon: null,
        label: "Tax Base Code",
        pinned: false,
      },
      {
        routerLink: "agreement-code",
        icon: null,
        label: "Agreement Code",
        pinned: false,
      },
      {
        routerLink: "commodity-code",
        icon: null,
        label: "Commodity Code",
        pinned: false,
      },
      {
        routerLink: "customs-procedure-code",
        icon: null,
        label: "Customs Procedure Code (CPC)",
        pinned: false,
      },
    ],
    tag: "iereference",
  },
  {
    routerLink: "reports",
    icon: "../assets/order-management/Order-Management.png",
    label: "Reports - Order Management",
    subMenus: [
      { routerLink: "order-detail-report", icon: null, label: "Order Detail" },
      { routerLink: "color-size-report", icon: null, label: "Colour/Size" },
      {
        routerLink: "scheduled-shipments-report",
        icon: null,
        label: "Scheduled Shipments",
      },
      {
        routerLink: "shipment-status-report",
        icon: null,
        label: "Shipment status Report",
      },
      {
        routerLink: "trim-sheet-report",
        icon: null,
        label: "Trim Sheet",
        pinned: false,
      },
      {
        routerLink: "year-season-orders-report",
        icon: null,
        label: "Year/Season Wise Orders",
      },
      {
        routerLink: "pending-events-report",
        icon: null,
        label: "Pending Events",
      },
      {
        routerLink: "order-quota-detail-report",
        icon: null,
        label: "Order/Quota Detail",
      },
      {
        routerLink: "stock-arrival-status-report",
        icon: null,
        label: "Stock Arrival Status",
      },
      {
        routerLink: "cost-of-production-report",
        icon: null,
        label: "Cost of Production",
      },
      {
        routerLink: "monthly-actual-shipments-report",
        icon: null,
        label: "Monthly Actual Shipments",
      },
      {
        routerLink: "purchase-order-list-report",
        icon: null,
        label: "List of P/O's",
      },
      {
        routerLink: "outstanding-purchase-order-list-report",
        icon: null,
        label: "List of Outstanding P/O's",
      },
      {
        routerLink: "post-order-cost-sheet-report",
        icon: null,
        label: "Post Order Cost Sheet",
      },
    ],
  },
];
