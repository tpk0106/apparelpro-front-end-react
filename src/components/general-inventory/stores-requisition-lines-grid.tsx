...
interface LinesGridProps {
  ...
  setLineItems: React.Dispatch<
    React.SetStateAction<GeneralRequisitionLineItemRow[]>
  >;
...
export default function StoresRequisitionLinesGrid({
  ...
  const handleUpdateLineCell = (
    index: number,
    field: keyof GeneralRequisitionLineItemRow,
    value: string | number,
  ) => {
    setLineItems((prev) => {
      const updated = [...prev];
  ...
  const handleExecuteCellBlurCheck = async (
    index: number,
    currentLine: GeneralRequisitionLineItemRow,
  ) => {
      ...
      {lineItems.length === 0 && (
        <Box sx={{ p: 3, textAlign: "center", border: "1px dashed #ccc", borderTop: "none" }}>
          <Typography variant="body2" sx={{ color: "text.secondary", fontStyle: "italic" }}>
            The requisition list is currently empty. Click "Add Item" above.
          </Typography>
        </Box>
      )}
    </Box>
