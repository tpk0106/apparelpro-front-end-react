...
export default function GoodsIssueNoteCascadeWorkspace() {
  ...
  const { data: pendingStrns = [], isLoading: isStrnsLoading } =
    useGetPendingStrnsByOrderQuery(
  ...
  const { data: lookupResult, isFetching: isLinesLoading } =
    useGetIssuableStrnLinesQuery(selectedStrn, !!selectedStrn);

  const { mutateAsync: commitGin, isPending: isSubmitting } =
    useCommitGinMutation();

  const isHeaderReady =
    !!selectedBuyer && !!selectedOrder && !!selectedStrn && !!lookupResult;
        ...
        {!isHeaderReady ? (
          ...
          <Box>
            ...
            >
              ...
              >
                Pending Material Lines &middot; To Dept.{" "}
                {lookupResult.departmentCode}
              </Typography>
