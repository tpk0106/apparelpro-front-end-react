...
const BuyerAddressesTable = ({
  ...
  const validateBuyerAddress = ({
    ...
    return {
      ...
      countryCode: validationRequired(countryCode || "")
        ? "Country for this Buyer required"
        : "",
  ...
  const handleCreateBuyerAddress: MRT_TableOptions<Address>["onCreatingRowSave"] =
    async ({ values, table }) => {
      ...
      values = { ...values, id: 0, buyerCode: buyerCode }; // update passed buyerCode with final payload to save
      ...
      console.log("create address api model", createAddressAPIModel);
      await createBuyerAddress(createAddressAPIModel);
