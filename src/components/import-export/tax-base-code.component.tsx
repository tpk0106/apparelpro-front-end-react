...
import {
  useGetTaxBaseCodes,
  useSaveTaxBaseCode,
  useDeleteTaxBaseCode,
} from "../../tanstack-hooks/import-export/tax-base-code.hooks";
import type { TaxBaseCode } from "../../interfaces/import-export/ImportExport";
import ConfirmDialog from "../common/confirm-dialog";

const EMPTY_FORM: TaxBaseCode = { code: "", description: "" };

const TaxBaseCodePage = () => {
  const { fieldSx } = useDropdownTheme();
  const { data: taxBaseCodes, isLoading } = useGetTaxBaseCodes();
  const saveMutation = useSaveTaxBaseCode();
  const deleteMutation = useDeleteTaxBaseCode();

  const [isFormOpen, setIsFormOpen] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [form, setForm] = useState<TaxBaseCode>(EMPTY_FORM);
  const [toDelete, setToDelete] = useState<TaxBaseCode | null>(null);

  const handleChange = (field: keyof TaxBaseCode) => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const openEditForm = (row: TaxBaseCode) => {
    setForm({ ...row });
    setIsEditing(true);
    setIsFormOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    saveMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast.success("Tax Base Code saved.");
          setIsFormOpen(false);
        },
        onError: (error) => toast.error(error.message || "Failed to save Tax Base Code."),
      },
    );
  };
  ...
  const handleConfirmDelete = () => {
    ...
    deleteMutation.mutate(toDelete.code, {
      onSuccess: () => {
        toast.success("Tax Base Code deleted.");
        setToDelete(null);
      },
      onError: (error) => toast.error(error.message || "Failed to delete Tax Base Code."),
    });
  };
    ...
    <Box sx={{ maxWidth: 820, mx: "auto", py: 4 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
        ...
        <Button variant="contained" startIcon={<AddIcon />} sx={primaryActionButtonSx} onClick={openAddForm}>
          <span style={themedButtonLabelStyle}>Add Tax Base Code</span>
        </Button>
      </Box>
      ...
      >
        ...
        {taxBaseCodes && taxBaseCodes.length === 0 && (
          <Box sx={{ py: 4, textAlign: "center" }}>
            <Typography sx={{ fontSize: "13px", color: "text.secondary" }}>
              No Tax Base Codes yet. Add one above.
            </Typography>
          </Box>
        )}
      </Box>
      ...
      >
        <DialogTitle sx={{ color: "#F4F6F8" }}>
          {isEditing ? "Edit Tax Base Code" : "Add Tax Base Code"}
        </DialogTitle>
        <DialogContent>
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button variant="contained" sx={primaryActionButtonSx} onClick={handleSave} disabled={saveMutation.isPending}>
            <span style={themedButtonLabelStyle}>Save Tax Base Code</span>
          </Button>
        </DialogActions>
      </Dialog>
...
export default TaxBaseCodePage;
