...
import {
  useGetDutyTaxCodes,
  useSaveDutyTaxCode,
  useDeleteDutyTaxCode,
} from "../../tanstack-hooks/import-export/duty-tax-code.hooks";
import type { DutyTaxCode } from "../../interfaces/import-export/ImportExport";
import ConfirmDialog from "../common/confirm-dialog";

const EMPTY_FORM: DutyTaxCode = { code: "", description: "" };

const DutyTaxCodePage = () => {
  const { fieldSx } = useDropdownTheme();
  const { data: dutyTaxCodes, isLoading } = useGetDutyTaxCodes();
  const saveMutation = useSaveDutyTaxCode();
  const deleteMutation = useDeleteDutyTaxCode();

  const [isFormOpen, setIsFormOpen] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [form, setForm] = useState<DutyTaxCode>(EMPTY_FORM);
  const [toDelete, setToDelete] = useState<DutyTaxCode | null>(null);

  const handleChange = (field: keyof DutyTaxCode) => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const openEditForm = (row: DutyTaxCode) => {
    setForm({ ...row });
    setIsEditing(true);
    setIsFormOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    saveMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast.success("Duty / Tax Code saved.");
          setIsFormOpen(false);
        },
        onError: (error) => toast.error(error.message || "Failed to save Duty / Tax Code."),
      },
    );
  };
  ...
  const handleConfirmDelete = () => {
    ...
    deleteMutation.mutate(toDelete.code, {
      onSuccess: () => {
        toast.success("Duty / Tax Code deleted.");
        setToDelete(null);
      },
      onError: (error) => toast.error(error.message || "Failed to delete Duty / Tax Code."),
    });
  };
    ...
    <Box sx={{ maxWidth: 820, mx: "auto", py: 4 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
        ...
        <Button variant="contained" startIcon={<AddIcon />} sx={primaryActionButtonSx} onClick={openAddForm}>
          <span style={themedButtonLabelStyle}>Add Duty / Tax Code</span>
        </Button>
      </Box>
      ...
      >
        ...
        {dutyTaxCodes && dutyTaxCodes.length === 0 && (
          <Box sx={{ py: 4, textAlign: "center" }}>
            <Typography sx={{ fontSize: "13px", color: "text.secondary" }}>
              No Duty / Tax Codes yet. Add one above.
            </Typography>
          </Box>
        )}
      </Box>
      ...
      >
        <DialogTitle sx={{ color: "#F4F6F8" }}>
          {isEditing ? "Edit Duty / Tax Code" : "Add Duty / Tax Code"}
        </DialogTitle>
        <DialogContent>
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button variant="contained" sx={primaryActionButtonSx} onClick={handleSave} disabled={saveMutation.isPending}>
            <span style={themedButtonLabelStyle}>Save Duty / Tax Code</span>
          </Button>
        </DialogActions>
      </Dialog>
...
export default DutyTaxCodePage;
