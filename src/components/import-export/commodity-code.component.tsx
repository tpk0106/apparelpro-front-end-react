...
import {
  useGetCommodityCodes,
  useSaveCommodityCode,
  useDeleteCommodityCode,
} from "../../tanstack-hooks/import-export/commodity-code.hooks";
import type { CommodityCode } from "../../interfaces/import-export/ImportExport";
import ConfirmDialog from "../common/confirm-dialog";

const EMPTY_FORM: CommodityCode = { code: "", description: "" };

const CommodityCodePage = () => {
  const { fieldSx } = useDropdownTheme();
  const { data: commodityCodes, isLoading } = useGetCommodityCodes();
  const saveMutation = useSaveCommodityCode();
  const deleteMutation = useDeleteCommodityCode();

  const [isFormOpen, setIsFormOpen] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [form, setForm] = useState<CommodityCode>(EMPTY_FORM);
  const [toDelete, setToDelete] = useState<CommodityCode | null>(null);

  const handleChange = (field: keyof CommodityCode) => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const openEditForm = (row: CommodityCode) => {
    setForm({ ...row });
    setIsEditing(true);
    setIsFormOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    saveMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast.success("Commodity Code saved.");
          setIsFormOpen(false);
        },
        onError: (error) => toast.error(error.message || "Failed to save Commodity Code."),
      },
    );
  };
  ...
  const handleConfirmDelete = () => {
    ...
    deleteMutation.mutate(toDelete.code, {
      onSuccess: () => {
        toast.success("Commodity Code deleted.");
        setToDelete(null);
      },
      onError: (error) => toast.error(error.message || "Failed to delete Commodity Code."),
    });
  };
    ...
    <Box sx={{ maxWidth: 820, mx: "auto", py: 4 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
        ...
        <Button variant="contained" startIcon={<AddIcon />} sx={primaryActionButtonSx} onClick={openAddForm}>
          <span style={themedButtonLabelStyle}>Add Commodity Code</span>
        </Button>
      </Box>
      ...
      >
        ...
        {commodityCodes && commodityCodes.length === 0 && (
          <Box sx={{ py: 4, textAlign: "center" }}>
            <Typography sx={{ fontSize: "13px", color: "text.secondary" }}>
              No Commodity Codes yet. Add one above.
            </Typography>
          </Box>
        )}
      </Box>
      ...
      >
        <DialogTitle sx={{ color: "#F4F6F8" }}>
          {isEditing ? "Edit Commodity Code" : "Add Commodity Code"}
        </DialogTitle>
        <DialogContent>
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button variant="contained" sx={primaryActionButtonSx} onClick={handleSave} disabled={saveMutation.isPending}>
            <span style={themedButtonLabelStyle}>Save Commodity Code</span>
          </Button>
        </DialogActions>
      </Dialog>
...
export default CommodityCodePage;
