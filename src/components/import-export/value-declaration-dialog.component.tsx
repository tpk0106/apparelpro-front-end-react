...
import {
  ...
  IconButton, MenuItem, TextField, Typography, Checkbox, FormControlLabel, CircularProgress,
} from "@mui/material";
...
import {
  useGetValueDeclarationByInvoice, useSaveValueDeclarationForInvoice, useDeleteValueDeclarationForInvoice,
  useDownloadValueDeclarationPrintPdfForInvoice,
} from "../../tanstack-hooks/import-export/value-declaration-invoice.hooks";
...
const NATURE_OF_TRANSACTION_OPTIONS = ["Sale", "Hire", "Gift"];
const OFFICE_USE_STATUS_OPTIONS = ["Satisfied", "Doubt", "Suspect fraud"];

const emptyHeader = (invoiceNumber: string): ValueDeclarationHeader => ({
  id: 0,
  invoiceNumber,
  companyAddressId: 0,
  year: "",
...
// Value Declaration Form (Sri Lanka Customs 308A) - invoice-scoped flow, one
// per Commercial Invoice, same pattern as Certificate of Origin/Boat Note.
// Exporter (box 1) is our own company (CompanyAddress dropdown); Importer
// (box 20) is resolved server-side from the linked invoice's own Consignee.
const ValueDeclarationDialog = ({ invoiceNumber, onClose }: Props) => {
  ...
  const { data: existing, isFetching: isLoadingExisting } = useGetValueDeclarationByInvoice(invoiceNumber);
  const saveMutation = useSaveValueDeclarationForInvoice();
  const deleteMutation = useDeleteValueDeclarationForInvoice();
  const printMutation = useDownloadValueDeclarationPrintPdfForInvoice();

  const [header, setHeader] = useState<ValueDeclarationHeader>(emptyHeader(invoiceNumber ?? ""));
  ...
  const handleSave = () => {
    if (!header.companyAddressId) {
      toast.error("Select the Exporter address.");
      return;
    }
    if (header.year && header.year.length !== 4) {
    ...
    saveMutation.mutate(
      ...
      {
        onSuccess: () => { toast.success("Value Declaration saved."); onClose(); },
  ...
  const handlePrint = () => {
    if (!invoiceNumber) return;
    printMutation.mutate({ invoiceNumber }, {
      onSuccess: () => toast.success(`Value Declaration ${invoiceNumber} printed.`),
  ...
  const handleConfirmDelete = () => {
    ...
    deleteMutation.mutate(invoiceNumber, {
      onSuccess: () => { toast.success("Value Declaration deleted."); setIsDeleteConfirmOpen(false); onClose(); },
  ...
  return (
    ...
    >
      <DialogTitle sx={{ color: DASHBOARD_COLORS.accentStrong }}>
        Value Declaration Form (Customs 308A) - Invoice {invoiceNumber}
      </DialogTitle>
      <DialogContent>
        {isLoadingExisting ? (
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 1 }}>
            <Box sx={{ display: "flex", gap: 2 }}>
              <TextField label="Year" size="small" sx={{ ...fieldSx, flex: 1, minWidth: 0 }}
                slotProps={{ htmlInput: { maxLength: 4, inputMode: "numeric", pattern: "[0-9]*" } }}
                value={header.year || ""}
            ...
            <Box sx={{ display: "flex", gap: 2 }}>
              <TextField select label="Exporter's Name & Address" size="small" sx={{ ...fieldSx, flex: 1, minWidth: 0 }}
                slotProps={{ select: modalSelectMenuProps }}
              ...
              <TextField label="Indenting Agent's Address" size="small" sx={{ ...fieldSx, flex: 1, minWidth: 0 }}
            ...
            <Box sx={{ display: "flex", gap: 2 }}>
              ...
              <TextField select label="Nature of Transaction" size="small" sx={{ ...fieldSx, flex: 1, minWidth: 0 }}
                slotProps={{ select: modalSelectMenuProps }}
                value={header.natureOfTransaction || ""} onChange={(e) => setField("natureOfTransaction", e.target.value)}>
                {NATURE_OF_TRANSACTION_OPTIONS.map((n) => <MenuItem key={n} value={n}>{n}</MenuItem>)}
              </TextField>
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 0.5 }}>
              ...
              {header.hasPreviousImportsLast3Months && (
                <TextField label="Details of previous imports" size="small" fullWidth sx={fieldSx}
                  value={header.previousImportsDetails || ""} onChange={(e) => setField("previousImportsDetails", e.target.value)} />
              )}
            </Box>
            ...
            <Typography sx={{ fontWeight: 600, mt: 1, color: DASHBOARD_COLORS.textPrimary }}>16. Complete Description of Goods</Typography>
            ...
            <Box sx={{ display: "flex", gap: 2 }}>
              <TextField select label="Appraiser's Comments" size="small" sx={{ ...fieldSx, flex: 1, minWidth: 0 }}
                slotProps={{ select: modalSelectMenuProps }}
                value={header.appraiserComments || ""} onChange={(e) => setField("appraiserComments", e.target.value)}>
                {OFFICE_USE_STATUS_OPTIONS.map((s) => <MenuItem key={s} value={s}>{s}</MenuItem>)}
              </TextField>
              <TextField select label="SC's Comments" size="small" sx={{ ...fieldSx, flex: 1, minWidth: 0 }}
                slotProps={{ select: modalSelectMenuProps }}
      ...
      <ConfirmDialog
        ...
        message={`Delete the Value Declaration for Invoice ${invoiceNumber}? This cannot be undone.`}
        confirmLabel="Delete"
        confirmColor="error"
...
export default ValueDeclarationDialog;
