...
import type { SanPrintReportLine } from "./san-print-report.types";
import { useApparelProTable } from "../../../../themes/useApparelProTable";

interface Props {
  data: SanPrintReportLine[];
  isLoading: boolean;
  isError: boolean;
}

export default function SanPrintReportGrid({
  data,
  isLoading,
  isError,
}: Props) {
  const columns = useMemo<MRT_ColumnDef<SanPrintReportLine>[]>(
    () => [
  ...
  // complete, fixed set of lines belonging to one already-committed SAN document —
  ...
  const table = useApparelProTable<SanPrintReportLine>({
    columns,
    data,
