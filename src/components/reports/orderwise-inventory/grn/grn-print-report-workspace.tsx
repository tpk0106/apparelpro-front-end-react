...
import GrnPrintReportGrid from "./grn-print-report-grid";
import KpiTile from "../../../common/kpi-tile";

import { format, parseISO } from "date-fns";
import {
  useDownloadGrnPrintPdfMutation,
  useGetGrnPrintDetailsQuery,
} from "../../../../tanstack-hooks/grn-print-report.hooks";
...
export default function GrnPrintReportWorkspace() {
  ...
  const {
    ...
    isError,
    error,
  } = useGetGrnPrintDetailsQuery(searchedGrnNumber, isReady);
      ...
      >
        <Box sx={{ textAlign: "center", mb: 3 }}>
          <Typography variant="h5" sx={workspaceHeadingSx}>
            Goods Received Note — Print (ORDERWISE)
          </Typography>
        </Box>
        ...
        {!isReady ? (
          <Alert severity="info" variant="outlined">
            Enter a GRN No and click Load to preview and print a committed
            Goods Received Note.
          </Alert>
        ) : isError ? (
          ...
          <>
            ...
            >
              <KpiTile
                label="GRN No"
                value={details?.header.grnNumber}
                loading={isLoading}
              ...
              <KpiTile
                label="Basis"
                value={details?.header.storeCode}
                loading={isLoading}
                size={{ xs: 12, sm: 6, md: 3, lg: 8 }}
              />
