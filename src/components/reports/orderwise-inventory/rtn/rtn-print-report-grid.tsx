...
import type { RtnPrintReportLine } from "./rtn-print-report.types";
import { useApparelProTable } from "../../../../themes/useApparelProTable";

interface Props {
  data: RtnPrintReportLine[];
  isLoading: boolean;
  isError: boolean;
}

export default function RtnPrintReportGrid({
  data,
  isLoading,
  isError,
}: Props) {
  const columns = useMemo<MRT_ColumnDef<RtnPrintReportLine>[]>(
    () => [
  ...
  // complete, fixed set of lines belonging to one already-committed RTN document —
  ...
  const table = useApparelProTable<RtnPrintReportLine>({
    columns,
    data,
