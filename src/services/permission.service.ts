...
const loadPermissionCatalog = async () => {
  return await client.get<Permission[]>(APPARELPRO_ENDPOINTS.PERMISSIONS.CATALOG);
};
...
const updateRolePermissions = async (request: UpdateRolePermissionsRequest) => {
  return await client.put(APPARELPRO_ENDPOINTS.PERMISSIONS.UPDATE_ROLE, request);
};

export { loadPermissionCatalog, loadRolePermissionMatrix, updateRolePermissions };
