...
import { DROPDOWN_LIST_DATA } from "../interfaces/definitions";
import type {
...
export const useCreateCurrency = (pagination: MRT_PaginationState) => {
  const dispatch = useDispatch();
  const queryClient = useQueryClient();
  ...
  return useMutation({
    ...
    onMutate: async (newCurrency: Currency) => {
      ...
      const previousCurrencies = queryClient.getQueryData([
        "currencies",
        pagination?.pageSize,
        pagination?.pageIndex,
      ]);
    ...
    onSuccess: () => {
      // 🚀 FIX: Translate MRT layout properties to your Redux PaginationData shape
      // 🚀 THE FIX: Dispatch using pageIndex directly since your PaginationData type now supports it!
      dispatch(
        loadAllCurrenciesStart({
